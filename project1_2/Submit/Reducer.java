		...
		try {
			...
			while ((input = reader.readLine()) != null) {
				...
				try {
					...
					//articleNameWithDateFromStream = items[0]; // articleName_date(yyyymmdd)
					//int indexOfLastDelimiter = articleNameWithDateFromStream.lastIndexOf("_");
					//articleNameFromStream = articleNameWithDateFromStream.substring(0, indexOfLastDelimiter);
					//dateFromStream = articleNameWithDateFromStream.substring(indexOfLastDelimiter + 1, indexOfLastDelimiter + 1 + 8);
					dateFromStream = items[1];
					viewsCount_string = items[2]; //views_count (hourly)
					viewsCount_int = Integer.parseInt(viewsCount_string);
					
					/*if (articleNameFromStream.equals("2016_NFL_draft")) {
						writer.write("\n" + articleNameFromStream + " " + dateFromStream + " " + viewsCount_string + "\n");
					...
					if (currentArticleName == null) { // the first line
						...
						// Aggregate article views_count of the same day
